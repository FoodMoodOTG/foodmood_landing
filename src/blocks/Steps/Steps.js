...
function Steps() {
  ...
  const renderPhones = () => {
    const phones = [
      { imgSrc: '/images/phone1.png', boxSrc: '/images/phoneBox1.png', alt: 'Phone 1', title: 'Create account', text: 'You might also receive a welcome email with additional instructions.' },
      { imgSrc: '/images/phone2.png', boxSrc: '/images/phoneBox2.png', alt: 'Phone 2', title: 'Customize', text: 'Upload dishes, description, setting prices, tracking orders, analytics viewing.' },
      { imgSrc: '/images/phone3.png', boxSrc: '/images/phoneBox3.png', alt: 'Phone 3', title: 'Launch & Enjoy', text: 'Placing QR codes on tables,  in social networks and on the website.' },
    ];
    ...
    return phones.map((phone, index) => (
      <div className={classes.phoneWrp} key={index}>
        ...
        <ImageWithFallback src={phone.boxSrc} className={classes.phoneBox} alt={`${phone.alt} Box`} />
        {/* Добавляем заголовок и текст под каждой картинкой */}
        <div className={classes.phoneText}>
          <Animate className={classes.index + ' font2'}>0{index + 1}.</Animate>
          <h3 className={classes.subtitle}>
            <AnimateText text={phone.title}/>
          </h3>
          <div className={classes.txt}><AnimateText text={phone.text}/></div>
          {isMob && (
  ...
  return (
    <div className={'container ' + classes.steps}>
      <Animate className={"title text-center"}>
        Just <span className={'underlineAfter'}>3 steps </span> {isMob && <br/>} to <span className={"font2"}>get started</span> in minutes.
      </Animate>
